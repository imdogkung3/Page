    ...
    setmetatable(FunctionsHandler, {__index = function(x, y)
            ...
            return {
                Register = function(D)
                    ...
                    function Result.RegisterMethod(J, K, L)
                        J.Methods[K] = {Name = K, Callback = L, Call = function(M, ...) return M.Callback(...) end, Events = {}}
                        return true
                    end
                    setmetatable(Result.Constants, {__newindex = function() assert(false, 'cannot change constant value!') end})
                    if x.Constants[Key] then
                        function Result.SaveConstant(J, K, L) return assert(false, 'constant name was used before!') end
                    ...
                    setmetatable(Result.CacheListener, {__newindex = function(L, M, O)
                        _ = L.__parent.Events[M] and L.__parent.Events[M](M, O)
